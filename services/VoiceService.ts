...
export async function speak(
  ...
  // Играем напрямую из base64 data URI — без записи файла
  const dataUri = `data:audio/mpeg;base64,${json.audio}`;
  ...
  const { sound } = await Audio.Sound.createAsync({ uri: dataUri }, { shouldPlay: false });
  _currentSound = sound;
